...
namespace MetaversePrototype.Tools
{
	...
	public class MPRandomBoundsInstantiator : MonoBehaviour
	{
		...
		[ShowIf("ScaleMode", (int)ScaleModes.Uniform)]
		...
		[ShowIf("ScaleMode", (int)ScaleModes.Vector3)]
		...
		public virtual void InstantiateRandomObject()
		{
			...
			obj.transform.position = MPBoundsExtensions.MPRandomPointInBounds(_collider.bounds);
			...
			switch (ScaleMode)
			{
				...
				case ScaleModes.Vector3:
					_newScale = MPMaths.RandomVector3(MinVectorScale, MaxVectorScale);
