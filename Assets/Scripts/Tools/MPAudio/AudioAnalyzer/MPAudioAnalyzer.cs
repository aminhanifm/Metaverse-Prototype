...
namespace MetaversePrototype.Tools
{
	...
	public class Beat
	{
		...
		[ShowIf("this.Modes == Modes.Raw || this.Modes == Modes.Normalized || this.Modes == Modes.BufferedRaw || this.Modes == Modes.BufferedNormalized")]
		public Color BeatColor = Color.cyan;
		public int BandID = 0;
		public float Threshold = 0.5f;
		public float MinimumTimeBetweenBeats = 0.25f;

		[ShowIf("BeatValueMode", BeatValueModes.Remapped)]
	...
	public class MPAudioAnalyzer : MonoBehaviour
	{
		...
		[ShowIf("Mode", (int)Modes.AudioSource)]
		[DisableInPlayMode]
		public AudioSource TargetAudioSource;
		[ShowIf("Mode", (int)Modes.Microphone)]
		...
		public virtual void Initialization()
		{
			...
			{
				Debug.LogError(this.name + " : this MPAudioAnalyzer needs a target audio source to operate.");
				return;
			}
			...
			if (Mode == Modes.Microphone)
			{
				...
				//UNCOMPENT_MICROPHONE string _microphone = Microphone.devices[MicrophoneID].ToString();
				//UNCOMPENT_MICROPHONE TargetAudioSource.clip = Microphone.Start(_microphone, true, _microphoneDuration, (int)_microphoneFrequency);
				//UNCOMPENT_MICROPHONE TargetAudioSource.Play();
			...
			while (true)
			{
				switch (Mode)
				{
					...
					case Modes.Microphone:
						...
						//UNCOMPENT_MICROPHONE microphoneSamples = Microphone.GetPosition(_microphone);
		...
		protected virtual void OnBeat(Beat beat, float rawValue)
		{
			...
			MPBeatEvent.Trigger(beat.Name, beat.CurrentValue);
