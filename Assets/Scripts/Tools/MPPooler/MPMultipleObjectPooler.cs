...
{	
	[Serializable]
	public class MPMultipleObjectPoolerObject
	{
	...
	public enum MPPoolingMethods { OriginalOrder, OriginalOrderSequential, RandomBetweenObjects, RandomPoolSizeBased }

	public class MPMultipleObjectPooler : MPObjectPooler
	{
		// the list of objects to pool
		public List<MPMultipleObjectPoolerObject> Pool;
		// the chosen pooling method
		public MPPoolingMethods PoolingMethod = MPPoolingMethods.RandomPoolSizeBased;
		...
		// a unique name that should match on all MPMultipleObjectPoolers you want to use together
		[ShowIf("MutualizeWaitingPools", true)]
		public string MutualizedPoolName = "";
		
		public List<MPMultipleObjectPooler> Owner { get; set; }
		...
		public override void FillObjectPool()
		{
			...
			{
				case MPPoolingMethods.OriginalOrder:
					...
					break;
				case MPPoolingMethods.OriginalOrderSequential:
					// we store our poolsizes in a temp array so it doesn't impact the inspector
					foreach (MPMultipleObjectPoolerObject pooledGameObject in Pool)
					{
		...
		public override GameObject GetPooledGameObject()
		{
			...
			switch (PoolingMethod)
			{
				case MPPoolingMethods.OriginalOrder:
					...
					break;
				case MPPoolingMethods.RandomPoolSizeBased:
					pooledGameObject =  GetPooledGameObjectPoolSizeBased();
					break;
				case MPPoolingMethods.RandomBetweenObjects:
		...
		protected virtual GameObject GetPooledGameObjectOriginalOrder()
		{
			...
			MPMultipleObjectPoolerObject searchedObject = GetPoolObject(Pool[_currentIndex].GameObjectToPool);
		...
		protected virtual void OrderSequentialResetCounter(MPMultipleObjectPoolerObject searchedObject)
		{
		...
		protected virtual GameObject GetPooledGameObjectPoolSizeBased()
		{
			...
			{	
				...
				else
				{
					...
					MPMultipleObjectPoolerObject searchedObject = GetPoolObject(_objectPool.PooledGameObjects[randomIndex].gameObject);
		...
		protected virtual MPMultipleObjectPoolerObject GetPoolObject(GameObject testedObject)
		{
			...
			int i=0;
			foreach(MPMultipleObjectPoolerObject poolerObject in Pool)
			{
		...
		{
			MPMultipleObjectPoolerObject searchedObject = GetPoolObject(testedObject);
			if (searchedObject != null)
			{
